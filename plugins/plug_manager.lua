...
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مگسيكو ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil) end  else
if not plugin_exists(matches[2]) then 
return '🗂*¦* لا يوجد ملف بهذا الاسم .\n\n'
else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua",'🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مگسيكو ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil)
end end end
...
if not we_sudo(msg) then return "🍥￤هذا الاوامر للمطور الاساسي فقط 🛠" end
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./"..matches[2].."/"..matches[3], '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مگسيكو ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil) end
...
return '👨🏾‍🔧¦ اصدار سورس مكسيكو : `v'..update..'` \n📡'
end
if (matches[1] == 'تحديث السورس' or matches[1] == 'تحديث السورس ™') and is_sudo(msg) then
if not we_sudo(msg)  then return "📛*￤* هذا الاوامر للمطور الاساسي فقط 🛠" end
local num_update = https.request('https://api.TOFIkO.com/version/') -- تحقق من التحديث اذا كان موجود او لا
if num_update > update then
sendMsg(msg.to.id,msg.id_,'🔛*¦* يوجد تحديث جديد الان \n📡*¦* جاري تنزيل وتثبيت التحديث  ...','md')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/inc/BOT.lua','./inc/BOT.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/inc/functions.lua','./inc/functions.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/inc/locks.lua','./inc/locks.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/plugins/cmd_help.lua','./plugins/cmd_help.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/plugins/plug_manager.lua','./plugins/plug_manager.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/plugins/proc_msgs.lua','./plugins/proc_msgs.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/plugins/rdod_bot.lua','./plugins/rdod_bot.lua')
download_file('https://raw.githubusercontent.com/MEXECOTEAM/TOFIkO/master/plugins/super_group.lua','./plugins/super_group.lua')
redis:set(TOFIkO..":VERSION",num_update)
sendMsg(msg.to.id,msg.id_,'🔝*┇* تم تحديث السورس الى اصدار *v'..redis:get(boss..":VERSION")..'*\n📟*┇* سوف يتم اعاده تشغيل السورس انتظر قليلا ...','md')
sleep(1)
RELOAD_BOT(msg)
else
...
return sendDocument(msg.to.id, msg.id_,0,1,nil,"./inc/buckup_all.lua",'🙋🏼‍♂️┇ مـرحبآ تم تگوين نسخه الاحتياطيه ¦ֆ \n🌿┇ لديگ حآليآ ('..#redis:smembers(boss..'group:ids')..') مـجمـوعهہ‏‏ في آلمـلف\n👨🏽‍🔧', dl_cb, nil)
end
...
end sendMsg(msg.to.id,msg.id_,'📦*¦* تم رفع النسخه الاحتياطيه\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هيه *'..#redis:smembers(boss..'group:ids')..'* 🌿\n✓','md')
