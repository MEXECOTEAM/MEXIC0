...
☤┇ فقط اضفني الئ مجموعتك واكتب تفعيل
☤┇ اختصاصي حمايه المجموعات مـن آلسـبآم وآلتوجيهہ‏‏ وآلتگرآر وآلخ...
☤┇ مـعرف مطـوري  : ]]..SUDO_USER:gsub([[\_]],'_')..[[ 📡
☻]],dl_cb,nil)
end
...
welcome = (redis:get(boss..'welcome:msg'..arg.chat_id) or "🔖*¦* بـٰٰٰٖٖٖۧگـﮧّٰعـ͜ާزٖيـِٰ̲زٖي 🤭♥️ 🤭♥️\n🔖*¦* ءاهـۛـٰཻـ͜لآ ﭑلَـٰٰٖـَمٌـٰٰٖجْـَٰۛمٌـﯢٰعـٰٚـِْـه💛 \n💂🏼‍♀️")
...
del_msg(msg.to.id,tonumber(msg.id_))
if redis:get(boss..'lock_woring'..msg.to.id) then
local msgx = "‼️¦ ممنوع ارسال الكليشه والا سوف سوف  اطردك  \n📛"
return sendMsg(msg.to.id,0,'*👤¦* العضو : ['..check_name(namecut(msg.from.first_name))..']\n🎟*¦* اليوزر : ['..usernamex..']\n'..msgx,'md')    
end
