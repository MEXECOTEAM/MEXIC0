...
sendMsg(msg.to.id,msg.id_,آوِڪٰྀہٰٰٖ بّہآيِٰہ •🌝🌸, html )
botrem(msg)
end   
if (r == "ضع اسم بوتك" or r==  ضع اسم للبوت © ) and is_sudo(msg) then
...
if we_sudo(msg) then
local text =  📯┇ مرحباً بك في بوت \n📮┇ البوت مخصص للمجموعات السوبرn...\n\n🔰┇ البوت يقوم بحمايه المجموعات \n🔱┇ ويوجد فيه جميع اوامر التقيد 

local keyboard = {
 {"ضع اسم بوتك☑️","ضع صوره للترحيب 🎖"},
...
sendMsg(msg.from.id,msg.id_,"📮┇ تـم آرسـآل الـرسـالـه 🔱\n📯¦ آلى : "..user_name.." 🏌🏻", html ) end
...
sendMsg(msg.to.id,0,"📥┇ تم اسـتـلام رسـآلتگ آلى آلمـطـور\n💠¦ سـآرد عليگ في آقرب وقت\n🏊‍♀ "..SUDO_USER, md )
...
return  الاحصائيات ⪼ 📈 \n\n👥*¦* عدد المجموعات المفعله في البوت ⪼  ..#group.. \n👤*¦* عدد المشتركين في البوت ⪼  ..#pv.. \n📡 
end
---------------[End Function data] -----------------------
...
return  ⚫️*¦* هذا الرد ليس مضاف في قائمه الردود ✖️ 
else
redis:hdel(boss.. replay: ..msg.to.id,msg.text)
redis:hdel(boss.. replay_photo:group: ..msg.to.id,msg.text)
redis:hdel(boss.. replay_voice:group: ..msg.to.id,msg.text)
...
if not is_sudo(msg) then return"🔸 للمطورين فقط ! 📡" end
redis:set(MEXIC0.. delrdall: ..msg.from.id,true) return "🔱┇ حسننا عزيزي  🤟\n📯┇ الان ارسل الرد لمسحها من  المجموعات 💠"
end
if r==  مسح رد  then
if not is_owner(msg) then return"🔸 للمدراء فقط ! 🔱┇" end redis:set(boss.. delrd: ..msg.from.id,true)
...
if (r==  الردود العامه  or r== الردود العامه ⚫️ ) and is_sudo(msg) then
local names = redis:hkeys(boss.. replay:all )
local photo =redis:hkeys(boss.. replay_photo:group: )
local voice = redis:hkeys(boss.. replay_voice:group: )
local animation = redis:hkeys(boss.. replay_animation:group: )
...
else sendMsg(msg.to.id,msg.id_,"✖️¦ لا يوجد صوره في بروفايلك ...\n-", md )
end end, nil)
elseif r=="اريد رابط الحذف" or r=="اريد رابط حذف" or r=="رابط حذف" or r=="رابط الحذف" then
return [[
🏄‍♂*¦* رابط حذف حـساب التيليگرام ⇩
☼┇ احذف وعيش حياتك شعندك باقي 
📜┇ بـالـسـلآمـه عـزيـزي ...
...
if msg.reply_id then
sendMsg(msg.to.id,msg.id_,'اوكـِـ̯͡ﮧﹻك حـِـ̯͡ﮧﹻبي🌞🌷 ','md')
sendMsg(msg.to.id,msg.reply_id,'حمبي شنو صايرلك صيت☻ بالنت وتكمز😪والشاشه لتخليني اهينك تره😠عندي السنا👋😎اذا اطلعه اخليك دوره براغي الكرامتك ...و لا تخليني اطبق عليك جهاد النعل😂️','md') end
